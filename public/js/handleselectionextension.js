...
/*                         if (confirm(`Isolate ${props.name} (${props.externalId})?`)) {
        ...
        this._button.setToolTip('Isolate selected elements');
        this._button.addClass('handleSelectionExtensionIcon');
        this._group.addControl(this._button);

        this._button2 = new Autodesk.Viewing.UI.Button('atbimButton');
        this._button2.onClick = (ev) => {
            getData().then((data) => {
                this._data = data;
                if (this._panel == null) {
                   // this._panel = new atbimDocking(viewer, viewer.container, 'miAtbimPanel', 'Panel Atbim');    
                }
               // this._panel.setVisible(!this._panel.isVisible());
               // this.drawData();
               // this._panel.resizeToContent(null);
            });
        };
