$(function() {
	...
	jQuery.get({
		...
		console.log(res);
		var num_electricidad = 0;
			var num_sanitario = 0;
			var num_arquitectura = 0;
			var num_general = 0;
			var num_climatizacion =0;
		for(var i=0;i<res.length;i++){
			cont = i+1;
			console.log( res[i].rol); //  /  /  /  / 
			if(res[i].rol == "CLIMATIZACIÓN"){
				num_climatizacion = num_climatizacion+1;
			}
			if(res[i].rol == "SANITARIO"){
				num_sanitario = num_sanitario+1;
			}
			if(res[i].rol == "ARQUITECTURA"){
				num_arquitectura = num_arquitectura+1;
			}
			if(res[i].rol == "GENERAL"){
				num_general = num_general+1;
			}
			if(res[i].rol == "ELECTRICIDAD"){
				num_electricidad = num_electricidad+1;
			}
			}
			...
			new Chart(ctx4, {
				type: 'horizontalBar',
				data: {
					labels: ['Clima', 'Sanitario', 'Arq', 'Gral', 'Elect'],
					datasets: [{
						label: 'Numero de Tareas',
						data: [num_climatizacion, num_sanitario, num_arquitectura, num_general, num_electricidad],
